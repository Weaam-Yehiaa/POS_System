...
import '../admin_module/menu_feature/data/data_source/admin_feature_remote_data.dart';
...
class ServicesLocator {
  void init() {
    ...
    getIt.registerFactory(
        () => AdminHomeBloc(getIt(), getIt(), getIt(), getIt()));
    getIt.registerFactory(() => AdminBloc());
    getIt.registerFactory<AdminNotesBloc>(
        () => AdminNotesBloc(getIt(), getIt(), getIt()));

    ///Remote DATA SOURCE
    getIt.registerLazySingleton<BaseAdminHomeRemoteDataSource>(
        () => AdminHomeRemoteDataSource());

    getIt.registerLazySingleton<BaseAdminNotesRemoteDataSource>(
        () => AdminNotesRemoteDataSource());

    // getIt.registerLazySingleton<BaseAdminMenuRemoteData>(
    //     () => AdminMenuRemoteDataImpl());

    ///REPOSITORY
    getIt.registerLazySingleton<BaseAdminHomeRepository>(
        () => AdminHomeRepository(getIt()));

    getIt.registerLazySingleton<BaseAdminNotesRepository>(
        () => AdminNotesRepository(getIt()));
    getIt
        .registerLazySingleton<BaseAdminMenuRepo>(() => AdminMenuRepo(AdminMenuRemoteDataImpl()));

    ///USE CASE
    getIt.registerLazySingleton(
        () => GetTimeAndDateUsingTimeStampUseCase(getIt()));
